...
{
    public class GenerateDocPartial
    {
        ...
        public GenerateDocPartial(Sdg sdg, IDataLayer dal, string coaName, bool english)//בנאי להפקת תעודה מלאה
        {
        ...
        public GenerateDocPartial(List<Sample> samples, IDataLayer dal, string coaName, bool english)//בנאי להפקת תעודה חלקית
        {
            //  Debugger.Launch();
            Init();
            this.dal = dal;
        ...
        public GenerateDocPartial(Sample sample, IDataLayer dal, string coaName, bool english)
        {
        ...
        private void CaseListSample()//פונקציה למקרה של דוח חלקי
        {
            ...
            foreach (Sample sample in Samples.Where(x => x.Status != "X").OrderBy(x => x.SampleId))//לב הדוח     
            {
            ...
            if (sdg.Client != null)
            {
                ...
                //        break;
                //}//מוסיף שדות נוספים לפי בחירת הלקוח
        ...
        private void CaseSdg()//פונקציה למקרה של דוח מלא 
        {
            ...
            //הגדרת רשימה של שמות הסמפלים לפי סדר
            var samplesName = new List<string>();
            foreach (Sample sample in sdg.Samples.Where(x => x.Status != "X").OrderBy(x => x.SampleId))
            {
                samplesName.Add(sample.Name);
        ...
        private List<Aliquot> OrderCoaColumns(List<Aliquot> chargeAliqNotCancel)
        {
            ...
            chargeAliqNotCancel = chargeAliqNotCancel.OrderBy(x => { return Array.IndexOf(ia, x.U_TEST_TEMPLATE_EXTENDED); }).ToList();
        ...
        private void AddRowAndColum<T>(string property, string header, DataTable dt, List<string> name)//פונקציה להוספת שדות נוספים לפי בקשת לקוח
        {
            ...
            if (newProperty != null)
            {
                ...
                for (int j = 0; j < dt.Rows.Count; j++)
                {
                    ...
                    object obj = newProperty.GetValue(sdg.Samples.Where(x => x.Status != "X").Where(s => s.Name == name[j]).ToList().FirstOrDefault(), null);//02/03/2014
                    dt.Rows[j][header] = obj;
                    i++;
                }
            }
        ...
        private void CaseSample(Sample sample)
        {
         //   Debugger.Launch();
            ...
            if (orderedList != null) chargeAliqNotCancel = orderedList;
            ...
            //Ashi - 28/10/18 BUGGGG
            if (_dtSample.Columns.Count < 3)
            {
                _dtSample.Columns.Add(new DataColumn("", Type.GetType("System.String")));
            }
        ...
        private void CaseAliq(Aliquot aliq, List<string> currentSample)
        {
            ...
            else
            {
                foreach (Test test in aliq.Tests)
                {
                    ...
                    CaseResult(test, currentSample, isCompleted);
                }
            }
        ...
        private void CaseResult(Test tests, List<string> currentSampleColumns, bool IsCompleted)
        {
            ...
            foreach (Result result in res)
            {
                if (result.REPORTED == "T" && onlyOne)
                {
                    ...
                    var inWork = "בעבודה";
                    if (_english) inWork = "In Progress";
                    if (_newCell)
                    {
                        if (IsCompleted)
                        {
                        ...
                        {
                            currentSampleColumns[_currentCell - 1] = inWork;
        ...
        public void DtToWord(DataTable fds)
        {
            ...
            //   {
            string save = sdg.LabInfo.U_COA_FOLDER;
        ...
        //    row = wd.Tables[1].Rows[3];
        //    if (sdg.ExternalReference != null) row.Cells[8].Range.Text = sdg.ExternalReference;
        ...
        private void TwoFirstTable(Document wd)
        {
            ...
            if (sdg.LabInfo.LabLetter == "C" || sdg.LabInfo.LabLetter == "M" || sdg.LabInfo.LabLetter == "A")//קוסמטיקה או כימיה או אלרגניים
            {
                row = wd.Tables[1].Rows[2];
            ...
            if (sdg.LabInfo.LabLetter == "W")//מים
            {
                ...
                row = wd.Tables[1].Rows[1];
                if (sdg.U_TXT_SAMPLING_TIME != null) row.Cells[8].Range.Text = sdg.U_TXT_SAMPLING_TIME + " " + sdg.U_TXT_RECIVED_TIME;//hila28.1.15;


                row = wd.Tables[1].Rows[2];

                row.Cells[8].Range.Text =
                Convert.ToDateTime(sdg.DeliveryDate).ToString("dd/MM/yy HH:mm") + " " + " C" + sdg.WaterTemperature.ToString() + "°";//hila 28.1.15
                ...
                row = wd.Tables[1].Rows[4];
                if (sdg.U_SAMPLING_SITE != null) row.Cells[2].Range.Text = sdg.U_SAMPLING_SITE;
                row = wd.Tables[1].Rows[4];
                if (sdg.U_MINISTRY_OF_HEALTH != null) row.Cells[5].Range.Text = sdg.U_MINISTRY_OF_HEALTH;
                row = wd.Tables[1].Rows[4];
                if (sdg.ExternalReference != null) row.Cells[8].Range.Text = sdg.ExternalReference;
            }
            ...
            if (sdg.LabInfo.LabLetter == "E")
            {
                ...
                if (sdg.U_SAMPLING_TYPE != null) row.Cells[5].Range.Text = sdg.U_SAMPLING_TYPE;
        ...
        //            {
        //                fld.Select();
        //                // string number = _coaName;
        //                // string snumber = number.Remove(number.IndexOf("("));//.Substring(number.IndexOf("(") + 1, (number.IndexOf(")") - number.IndexOf("(") + 1) - 2);                                 
        //                //if (Convert.ToInt32(snumber) > 1)
        //                //{
        //                //    int befor = Convert.ToInt32(snumber) - 1;
        //                //   // var beforStr = string.Format("({0}){1}", befor, _coaName.Substring(0, _coaName.IndexOf("(")));
        //                //    //if (_english)
        //                //    //{
        //                //    //    wordApp.Selection.TypeText(beforStr + " This certificate replaces the certificate number" );
        //                //    //}
        //                //    //else
        //                //    //{
        //                //    //     wordApp.Selection.TypeText("תעודה זאת מחליפה את תעודה מספר " + beforStr);  
        ...
        private void PermanentFields(Document wd, Application wordApp)
        {
            ...
            foreach (Field fld in flds)
            {
                ...
                if (fieldText.StartsWith(" MERGEFIELD"))
                {
                    ...
                    {
                        fld.Select();
                        if (sdg.U_COA_REMARKS != null) wordApp.Selection.TypeText(sdg.U_COA_REMARKS);
                        else
                        {
                            wordApp.Selection.TypeText(" ");
                        }
                    ...
                    if (fieldName == "sdgSpecification")//hila 28.1.15
                    {
                        ...
                        else
                        {
                            ...
                            if (sdg.U_SPECIFICATION != null) { wordApp.Selection.TypeText(sdg.U_SPECIFICATION); }
                            else
                            {
                                wordApp.Selection.TypeText(" , ");
                            }
            ...
            foreach (Field myMergeField in wd.Fields)//שדות נוספים של גוף המסמך
            {
                ...
                if (fieldText.StartsWith(" MERGEFIELD"))
                {
                    ...
                    String fieldName = fieldText.Substring(11, endMerge - 11);
                    fieldName = fieldName.Trim();
                    if (sdg.LabInfo.LabLetter != "C" && fieldName == "remark")
                    {
                        myMergeField.Select();
                        if (sdg.U_COA_REMARKS != null) wordApp.Selection.TypeText(sdg.U_COA_REMARKS);
                        else
